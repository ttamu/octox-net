...
impl SysCalls {
    ...
    pub fn tcpconnect() -> Result<()> {
        ...
        #[cfg(all(target_os = "none", feature = "kernel"))]
        {
            ...
            use crate::net::tcp::socket::{IpEndpoint, State};
            ...
            loop {
                ...
                match state {
                    ...
                    State::SynSent | State::SynReceived => {
                        // Still connecting, sleep and retry
                        if p.inner.lock().killed {
                            return Err(Interrupted);
                        }
    ...
    pub fn tcplisten() -> Result<()> {
        ...
        #[cfg(all(target_os = "none", feature = "kernel"))]
        {
            ...
            use crate::net::tcp::socket::IpEndpoint;
    ...
    pub fn tcpsend() -> Result<usize> {
        ...
        #[cfg(all(target_os = "none", feature = "kernel"))]
        {
            ...
            crate::net::tcp::socket_get_mut(sock, |socket| socket.send_slice(&buf))?
        }
    }
    ...
    pub fn tcprecv() -> Result<usize> {
        ...
        #[cfg(all(target_os = "none", feature = "kernel"))]
        {
            use crate::net::tcp::socket::State;
            ...
            let p = Cpus::myproc().unwrap();
            loop {
                let (may_recv, state) = crate::net::tcp::socket_get(sock, |s| {
                    (s.may_recv(), s.state())
                })?;
    ...
    pub fn tcpclose() -> Result<()> {
        ...
        #[cfg(all(target_os = "none", feature = "kernel"))]
        {
            ...
            // Wait for close to complete, then free the socket
            let p = Cpus::myproc().unwrap();
            loop {
                let state = crate::net::tcp::socket_get(sock, |s| s.state())?;
                if state == crate::net::tcp::socket::State::Closed {
    ...
    pub fn tcpaccept() -> Result<usize> {
        ...
        #[cfg(all(target_os = "none", feature = "kernel"))]
        {
            ...
            loop {
                ...
                let has_pending = crate::net::tcp::socket_get(sock, |s| {
