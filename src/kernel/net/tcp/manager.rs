...
pub fn tcp_input(src_ip: IpAddr, dst_ip: IpAddr, data: &[u8]) -> Result<()> {
    // Parse TCP packet
    // Note: checksum validation disabled because QEMU virtio-net uses checksum offload
    let packet = Packet::new_checked(data)?;
    let repr = Repr::parse(&packet, src_ip, dst_ip, false)?;
    ...
    crate::trace!(
        ...
        if repr.payload.is_empty() {
            ...
            alloc::format!(" {} bytes", repr.payload.len())
        }
    );
...

    {
        ...
        // First pass: look for exact 4-tuple match (non-LISTEN sockets)
        let mut listen_idx: Option<usize> = None;
        let mut matched_idx: Option<usize> = None;

        for (i, entry) in sockets.iter().enumerate() {
            if let Some(socket) = &entry.socket {
                if socket.accepts(src_ip, repr.src_port, dst_ip, repr.dst_port, &repr) {
        ...
        // Use exact match if found, otherwise use LISTEN socket
        let target_idx = matched_idx.or(listen_idx);

        if let Some(idx) = target_idx {
            let socket = sockets[idx].socket.as_mut().unwrap();
    ...
    crate::trace!(TCP, "[tcp] no matching socket, sending RST");
    let rst = rst_for_segment(&repr);
    send_segment(dst_ip, src_ip, &rst)?;

    // Poll for any pending transmissions
    let _ = tcp_poll();
        ...
        for entry in sockets.iter_mut() {
            if let Some(socket) = &mut entry.socket {
                ...
                if let Some((repr, payload)) = socket.dispatch(timestamp_ms) {
                    ...
                    if local.port != 0 && local.addr.0 != 0 && !remote.is_unspecified() {
...
fn send_segment_with_payload(
    ...
    crate::trace!(
        ...
        if payload.is_empty() {
            ...
            alloc::format!(" {} bytes", payload.len())
        }
    );
...
fn format_flags(repr: &Repr) -> alloc::string::String {
    use super::wire::Control;
    let mut flags = alloc::vec::Vec::new();

    match repr.control {
        Control::Syn => flags.push("SYN"),
        Control::Fin => flags.push("FIN"),
        Control::Rst => flags.push("RST"),
        Control::Psh => flags.push("PSH"),
        Control::None => {}
    }
